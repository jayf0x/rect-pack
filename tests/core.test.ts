...
import { packGrid } from "../src/core";
import type { GridInput } from "../src/types";

const EPS = 1e-9;

const equalItems = (n: number): GridInput[] =>
...
describe("packGrid", () => {
  test("single item fills the whole unit square", () => {
    expect(packGrid([{ id: 0 }], { cols: 7, rows: 7 })).toEqual([
  ...
  test("no overlap between any two rects", () => {
    ...
    for (let i = 0; i < placed.length; i++) {
      for (let j = i + 1; j < placed.length; j++) {
        ...
        expect(
          overlapX > EPS && overlapY > EPS,
          `overlap between id ${a.id} and id ${b.id}`,
        ).toBe(false);
      }
    }
  });
  ...
  test("extreme weight skew still tiles exactly and keeps area proportional", () => {
    const items: GridInput[] = [
      { id: "big", weight: 1000 },
      ...equalItems(30).map((it) => ({ ...it, weight: 0.01 })),
    ];
    const placed = packGrid(items, { cols: 6, rows: 6 });
    const area = placed.reduce((s, p) => s + p.w * p.h, 0);
  ...
  test("deterministic: same input produces identical output every time", () => {
    ...
    expect(packGrid(items, { cols: 5, rows: 5 })).toEqual(
      packGrid(items, { cols: 5, rows: 5 }),
    );
  });
